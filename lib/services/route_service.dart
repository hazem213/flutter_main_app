...
class RouteService {
  ...
  static ActivityType detectActivity(List<LocationPoint> points) {
    if (points.length < 2) return ActivityType.stationary;

    // ✅ آخر نقطتين بس = أسرع استجابة
    ...
    if (seconds <= 0) return ActivityType.stationary;

    final kmh = (dist / seconds) * 3.6;

    if (kmh < 1.0)  return ActivityType.stationary; // 🧍 واقف
    if (kmh < 7.0)  return ActivityType.walk;        // 🚶 مشي
    if (kmh < 12.0) return ActivityType.jog;         // 🏃 هرولة
    return ActivityType.run;                          // 🏃 جري
  }
}